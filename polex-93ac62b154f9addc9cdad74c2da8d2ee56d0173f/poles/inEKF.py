...
class inEKF:
    def __init__(self, start, posrange, angrange, polemeans, polevar, T_w_o = np.identity(4)):
        ...
        self.V = np.diag([polevar, polevar]) # not sure if this is correct
    ...
    def update_motion(self, u, Q):
        ...
        # predict covariance
        AdjX = np.block([[self.mu[0:2, 0:2], np.array([[self.mu[1,2]], [-self.mu[0,2]]])], [0, 0, 1]]) # ERROR
    ...
    def update_measurement(self, poleparams):
        ...
        for i in range(n):
            ...
            # THIS MIGHT BE WRONG: should just skip index[i] == self.kdtree.n !!
            if index[i] < self.kdtree.n: 
                H[2 * i, :] = [self.polemeans[index[i], 1], -1, 0]
                H[2 * i + 1, :] =  [-self.polemeans[index[i], 0], 0, -1]
                v[(2*i):(2*i+1) + 1, 0] = polepos_w[0:2, i] - self.polemeans[index[i], 0:2].T
